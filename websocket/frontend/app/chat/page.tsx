...
export default function ChatPage() {
  ...
  const { messages, isConnected, historyLoaded, sendMessage, subscribe, unsubscribe } = useWebSocketContext();
      ...
      <div key={msg.id} className={`flex ${msg.isOwn ? "justify-end" : "justify-start"} mb-4`}>
        <div className={`max-w-xs lg:max-w-md ${msg.isOwn ? "order-2" : "order-1"}`}>
          <div className="flex items-baseline space-x-2 mb-1">
            <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{msg.username || "Unknown"}</span>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <div className="max-w-4xl mx-auto">
          ...
          {historyLoaded && messages.length === 0 && (
            <div className="text-center text-gray-500 dark:text-gray-400 mt-8">No messages yet. Start chatting!</div>
          )}
