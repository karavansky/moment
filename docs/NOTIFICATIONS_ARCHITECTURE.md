...
## 7. Управление флагом `pushNotificationsEnabled`

Флаг `users.pushNotificationsEnabled` в базе данных контролирует, будут ли отправляться push-уведомления пользователю. По умолчанию он установлен в `true`.

### Автоматическое управление флагом:

1. **Включение при регистрации подписки** (`lib/push-notifications.ts` → `saveSubscription`):
   - Когда пользователь регистрирует новую push-подписку (через `usePushNotifications` hook), флаг автоматически включается
   - Это решает проблему, когда флаг был отключен вручную или из-за других причин, но пользователь снова хочет получать уведомления

2. **Обработка мертвых токенов** (`app/api/staff/verify-push/route.ts`):
   - Когда директор проверяет работника через UI "Verify Push"
   - Если токен возвращает статус 410 Gone или 404 (устарел, отозван браузером)
   - Система **только удаляет мертвую подписку** из таблицы `push_subscriptions`
   - Флаг `pushNotificationsEnabled` НЕ изменяется - он остается в том состоянии, которое было
   - При следующей попытке отправить уведомление: если нет подписки, уведомление просто пропускается (нет ошибки)

### Важно:

- **НЕ синхронизируем флаг из клиента** (`lib/users.ts` → `updateDeviceSyncStatus`):
  - Раньше `useDeviceSync` отправлял состояние подписки из браузера и перезаписывал флаг в БД
  - Это создавало race condition: если между удалением старого токена и регистрацией нового происходил visibility change, флаг мог быть ошибочно отключен
  - Теперь управление флагом полностью серверное (автоматическое)

### Сценарий работы после миграции сервера:

1. После переезда на новый сервер старые push-токены становятся невалидными
2. Директор проверяет работника → токен 410 Gone → подписка удаляется из БД
3. Работник открывает приложение → `usePushNotifications` регистрирует новую подписку
4. Backend автоматически включает `pushNotificationsEnabled = true` (если был false)
5. Push-уведомления снова работают

### Случаи когда флаг может быть `false`:

- **Никогда автоматически** - флаг больше не устанавливается в `false` автоматически
- **Только вручную** - администратор может вручную отключить через SQL или будущий UI
- **Будущее улучшение** - можно добавить UI настройку в профиле пользователя для самостоятельного управления
