...
const API_BASE_URL = (() => {
  ...
  if (typeof window === "undefined") {
    return process.env.NODE_ENV === "development" ? "/api" : process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001";
  }
...
export class ApiClient {
  ...
  static async register(username: string, password: string, host?: string) {
    return this.fetchJSON<{ sessionId: string; username: string; expiresAt?: number }>("/register", {
      method: "POST",
  ...
  static getWebSocketURL(sessionId: string): string {
    ...
    const backendUrl = typeof window !== "undefined" ? `ws://${window.location.hostname}:3003` : "ws://localhost:3003";
