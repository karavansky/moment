...
### Модели планирования (Scheduling)

#### Appointment.swift → таблица `appointments`

| Колонка         | Тип         | Описание                                       |
| --------------- | ----------- | ---------------------------------------------- |
| `appointmentID` | varchar(21) | PK, nanoid                                     |
| `firmaID`       | varchar(21) | FK → organisations                             |
| `userID`        | varchar(21) | FK → users                                     |
| `clientID`      | varchar(21) | FK → clients                                   |
| `workerId`      | varchar(21) | Primary worker ID                              |
| `date`          | timestamptz | Дата встречи                                   |
| `isFixedTime`   | boolean     | Фиксированное время (nullable)                 |
| `startTime`     | timestamptz | Время начала                                   |
| `endTime`       | timestamptz | Время окончания                                |
| `duration`      | integer     | Длительность в минутах                         |
| `fahrzeit`      | integer     | Время в пути (nullable)                        |
| `isOpen`        | boolean     | Статус открытия (nullable)                     |
| `openedAt`      | timestamptz | Время открытия (nullable)                      |
| `closedAt`      | timestamptz | Время закрытия (nullable)                      |
| `latitude`      | double      | Широта (nullable)                              |
| `longitude`     | double      | Долгота (nullable)                             |
| `createdAt`     | timestamptz | **Время создания записи** (обязательное)       |
| `editedAt`      | timestamptz | **Время последнего редактирования** (nullable) |

> **Важно**: Поля `createdAt` и `editedAt` добавлены в марте 2026. При создании записи `createdAt` устанавливается автоматически. При любом обновлении записи `editedAt` автоматически обновляется до текущего времени.

**Связанные таблицы:**
- `appointment_workers` (many-to-many с `workers`)
- `appointment_services` (many-to-many с `services`)
...
### 1. Appointment: поддержка createdAt и editedAt

**Добавлены поля в модель Appointment:**
- `createdAt: Date` — обязательное поле, автоматически устанавливается при создании записи
- `editedAt: Date?` — опциональное поле, автоматически обновляется при любом изменении записи

**Изменения в коде:**

1. **Appointment.swift** — обновлена модель:
   ```swift
   @Field(key: "createdAt")
   var createdAt: Date
...
2. **AppointmentController.swift** — автоматическая установка timestamps:
   ```swift
   // При создании (create):
   let createdAt = Date()
   INSERT INTO appointments (..., "createdAt") VALUES (..., \(bind: createdAt))
...
3. **AppointmentDTO** — добавлены поля для сериализации:
   ```swift
...
4. **decodeAppointmentRow** — маппинг из SQL:
   ```swift
...
ALTER TABLE appointments ADD COLUMN "editedAt" TIMESTAMP;
UPDATE appointments SET "createdAt" = "date" WHERE "createdAt" IS NULL;
CREATE INDEX idx_appointments_created_at ON appointments("createdAt");
CREATE INDEX idx_appointments_edited_at ON appointments("editedAt");
```

**Frontend интеграция:**
- Frontend отображает `createdAt` в заголовке встречи с локализованным форматом
- При обновлении через Next.js или Vapor API поле `editedAt` автоматически обновляется

### 2. Транспортная система (Transport API)
