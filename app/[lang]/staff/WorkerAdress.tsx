...
export const WorkerAdress = memo(function WorkerAdress({
  worker,
  isCreateNew = false,
  className,
}: WorkerAdressProps) {
  ...
  const [cityQuery, setCityQuery] = useState(worker.city || '')
  const [streetQuery, setStreetQuery] = useState(worker.street || '')
  ...
  const isChanged =
    addressData.street !== (worker.street || '') ||
    addressData.city !== (worker.city || '') ||
    addressData.zipCode !== (worker.postalCode || '') ||
    addressData.houseNumber !== (worker.houseNumber || '') ||
    addressData.country !== (worker.country || '') ||
    addressData.district !== (worker.district || '')
  ...
  const listCities = useAsyncList<Character>({
    initialSelectedKeys: [worker.city ? worker.city : ''],
    initialFilterText: '', // worker.city || '', - Избегаем загрузки при монтировании
  ...
  const listStreets = useAsyncList<{ id: number; street: string; district: string }>({
    initialFilterText: '', // worker.street || '', - Избегаем загрузки при монтировании
  ...
  const onSubmit = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const workerData: Worker = {
        ...
        latitude: addressCoordinates?.lat ?? worker.latitude,
        longitude: addressCoordinates?.lng ?? worker.longitude,
      }

      if (isCreateNew) {
        addWorker(workerData)
        alert(`Worker ${worker.name} ${worker.surname} created successfully!`)
      } else {
        updateWorker(workerData)
        alert(`Address saved successfully for ${worker.name} ${worker.surname}!`)
      }
    },
    [addressData, country, worker, updateWorker, addWorker, addressCoordinates, isCreateNew]
  )
  ...
  const handleReset = useCallback(() => {
    ...
    setCityQuery(worker.city || '')
    setStreetQuery(worker.street || '')
  }, [
    worker.country,
    worker.city,
    worker.street,
