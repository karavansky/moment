...
interface AppReportProps {
  ...
  setFormData: (formData: Appointment) => void
  errors: any
  setErrors: (errors: any) => void
  selectedDate?: Date
}

function AppReport({ formData, setFormData, errors, setErrors, selectedDate }: AppReportProps) {
  ...
  React.useEffect(() => {
    console.log('[AppReport Debug] Loading sessions for appointment:', appointment)
    if (isOpen && appointment) {
      ...
      if (active) {
        console.log('[AppReport Debug] Using active session photos:', active.photos)
        setPhotos(active.photos || [])
      } else {
        const allPhotos = sessions.flatMap(s => s.photos || [])
        console.log(
          '[AppReport Debug] Using ALL photos from sessions:',
          allPhotos,
          'Sessions:',
          sessions
        )
      ...
      if (!appointment.isOpen) {
        ...
        if (orphaned.length > 0) {
          console.log('[AppReport Debug] Found orphaned sessions to autoclose:', orphaned)
        }
        orphaned.forEach(s => {
        ...
        {(() => {
          const closedSessionsSeconds = reportSessions
            ...
            .reduce(
              (acc, s) =>
                acc +
                Math.floor((new Date(s.closeAt!).getTime() - new Date(s.openAt!).getTime()) / 1000),
              0
            )
              ...
              <div key={session.id}>
                <div className="p-1 bg-default-50 rounded-lg space-y-1">
                  ...
                  <div className="flex items-baseline justify-between gap-2 text-sm">
                    ...
                    {(session.openAddress || session.openDistanceToAppointment != null) && (
                      <button
                        ...
                        onClick={() => {
                          if (session.openLatitude != null && session.openLongitude != null) {
                            setMapModal({
                  ...
                  <div className="flex items-baseline justify-between gap-2 text-sm">
                    ...
                    {(session.closeAddress || session.closeDistanceToAppointment != null) && (
                      <button
                        type="button"
                        className="text-xs text-default-500 text-right truncate hover:text-primary cursor-pointer inline-flex items-center gap-0.5 transition-colors"
                        onClick={() => {
                          if (session.closeLatitude != null && session.closeLongitude != null) {
                            setMapModal({
                ...
                <Modal.Body className="relative flex-1 flex items-center justify-center bg-default-100 rounded-lg overflow-hidden group min-h-0">
                  <AuthenticatedImage
                    src={getPhotoUrl(selectedPhoto?.url || '', {
                      ...
                      reportId:
                        currentReportId ||
                        reportSessions.find(s =>
                          (s.photos || []).some(p => p.id === selectedPhoto?.id)
                        )?.id ||
                        '',
                    })}
                    alt="Report photo"
                  ...
                  <div className="absolute bottom-0 left-0 right-0 rounded-b-lg p-2 bg-transparent backdrop-blur-sm">
                    <Input
                      ...
                      onChange={e => {
                        if (selectedPhoto) handlePhotoNoteChange(selectedPhoto.id, e.target.value)
                      }}
                    />
                  </div>
                </Modal.Body>
