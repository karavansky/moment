...
export function useWebSocket({ sessionId, username, onConnectionChange, subscribeToTypes = [] }: UseWebSocketOptions) {
  ...
  const connect = useCallback(async () => {
    console.log("[useWebSocket] connect() called with sessionId:", !!sessionIdRef.current, "username:", usernameRef.current);
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      console.log("[useWebSocket] Already connected, skipping. readyState:", wsRef.current.readyState);
      return;
    }

    if (wsRef.current?.readyState === WebSocket.CONNECTING) {
      console.log("[useWebSocket] Already connecting, skipping. readyState:", wsRef.current.readyState);
      return;
    }
    ...
    try {
      ...
      const subscribeNext = (ws: WebSocket) => {
        if (pendingSubscriptionsRef.current.length === 0) {
          console.log("[WebSocket] All subscriptions completed:", Array.from(subscribedTypesRef.current));
          return;
        }
        ...
        setTimeout(() => {
          if (pendingSubscriptionsRef.current[0] === messageType) {
            console.warn("[WebSocket] Subscription timeout for:", messageType, "- proceeding to next");
      ...
      ws.onopen = () => {
        ...
        if (subscribeToTypesRef.current.length > 0) {
          ...
          console.log("[WebSocket] Starting sequential subscriptions:", pendingSubscriptionsRef.current);
          subscribeNext(ws);
        }
      };
...
*/
          ...
          if (data.t === "system" || data.t === "message" || data.t === "join" || data.t === "leave") {
            ...
            setMessages((prev) => {
              ...
              console.log("[WebSocket] 📝 Added message to state. Total messages:", newMessages.length);
              return newMessages;
            });
          }
        } catch (error) {
  ...
  const sendMessage = useCallback((content: string) => {
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      console.error("[WebSocket] Cannot send - WebSocket is not connected. State:", wsRef.current?.readyState);
    }
  }, []);
  ...
  useEffect(() => {
    console.log("History loading effect:", { sessionId: !!sessionId, username: !!username, historyLoaded });
  ...
  useEffect(() => {
    console.log("WebSocket connection effect:", { sessionId: !!sessionId, username: !!username, historyLoaded });
